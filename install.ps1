...
if (Test-Path (Join-Path $installDir '.git')) {
    ...
    $currentHash = git -C $installDir log --oneline -1 2>&1
    git -C $installDir fetch origin 2>&1 | Out-Null
    $remoteHash  = git -C $installDir log --oneline -1 origin/HEAD 2>&1
...
$batContent = "@echo off`r`ncd /d `"%~dp0`"`r`nstart `"`" /b .venv\Scripts\pythonw.exe -m gsd_monitor`r`n"
Set-Content -Path (Join-Path $installDir 'start.bat') -Value $batContent -Encoding ASCII -NoNewline
